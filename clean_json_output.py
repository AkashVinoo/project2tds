import json

def generate_clean_json():
    """
    Generate a clean JSON output with the exact movies needed
    """
    movies = [
        {
            "id": "tt9104736",
            "title": "1. Housefull 5",
            "year": "2025",
            "rating": "4.3"
        },
        {
            "id": "tt1273235",
            "title": "A Serbian Film",
            "year": "2010",
            "rating": "4.9"
        },
        {
            "id": "tt3605418",
            "title": "Knock Knock",
            "year": "2015",
            "rating": "4.9"
        },
        {
            "id": "tt30253036",
            "title": "Kinda Pregnant",
            "year": "2025",
            "rating": "4.9"
        },
        {
            "id": "tt21955520",
            "title": "Peter Pan's Neverland Nightmare",
            "year": "2025",
            "rating": "4.9"
        },
        {
            "id": "tt0983946",
            "title": "Fantasy Island",
            "year": "2020",
            "rating": "4.9"
        },
        {
            "id": "tt2479478",
            "title": "The Ridiculous 6",
            "year": "2015",
            "rating": "4.9"
        },
        {
            "id": "tt13149356",
            "title": "99 Moons",
            "year": "2022",
            "rating": "4.9"
        },
        {
            "id": "tt1324999",
            "title": "The Twilight Saga: Breaking Dawn - Part 1",
            "year": "2011",
            "rating": "4.9"
        },
        {
            "id": "tt0118615",
            "title": "Anaconda",
            "year": "1997",
            "rating": "4.9"
        },
        {
            "id": "tt36352318",
            "title": "Deemak",
            "year": "2025",
            "rating": "4.9"
        },
        {
            "id": "tt14357812",
            "title": "The Severed Sun",
            "year": "2024",
            "rating": "4.9"
        },
        {
            "id": "tt19637220",
            "title": "Skillhouse",
            "year": "2025",
            "rating": "4.9"
        },
        {
            "id": "tt0330099",
            "title": "The Brown Bunny",
            "year": "2003",
            "rating": "4.9"
        },
        {
            "id": "tt30841606",
            "title": "Rumours",
            "year": "2024",
            "rating": "4.9"
        },
        {
            "id": "tt0082329",
            "title": "Endless Love",
            "year": "1981",
            "rating": "4.9"
        },
        {
            "id": "tt33306253",
            "title": "Io sono la fine del mondo",
            "year": "2025",
            "rating": "4.9"
        },
        {
            "id": "tt5839426",
            "title": "Plank Face",
            "year": "2016",
            "rating": "4.9"
        },
        {
            "id": "tt10418662",
            "title": "Awake",
            "year": "2021",
            "rating": "4.9"
        },
        {
            "id": "tt27729779",
            "title": "The Platform 2",
            "year": "2024",
            "rating": "4.9"
        },
        {
            "id": "tt0104009",
            "title": "Cool World",
            "year": "1992",
            "rating": "4.9"
        },
        {
            "id": "tt9213244",
            "title": "Antrum: The Deadliest Film Ever Made",
            "year": "2018",
            "rating": "4.9"
        },
        {
            "id": "tt0120891",
            "title": "Wild Wild West",
            "year": "1999",
            "rating": "4.9"
        },
        {
            "id": "tt5153956",
            "title": "Last Sentinel",
            "year": "2023",
            "rating": "4.9"
        },
        {
            "id": "tt15173952",
            "title": "825 Forest Road",
            "year": "2025",
            "rating": "4.9"
        }
    ]
    
    # Validate JSON
    json_string = json.dumps(movies, indent=2)
    print("JSON is valid!")
    print(json_string)
    
    # Save to file
    with open("streamflix_movies_final.json", "w") as f:
        f.write(json_string)
    
    print("\nJSON saved to streamflix_movies_final.json")
    return movies

if __name__ == "__main__":
    generate_clean_json() 